...
export interface Homecare extends mongoose.Document {
  name: string;
  href: string;
  image_url: string;
  category: string;
...
const HomecareSchema = new mongoose.Schema<Homecare>({
  name: {
    type: String,
...
export default mongoose.models.Homecare ||
  mongoose.model<Homecare>("Homecare", HomecareSchema);
